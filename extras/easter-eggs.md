...
D1 与 Tyson 初次探索金库时输入 `THE END`，游戏会按照字面含义直接结束当前运行。

它不会开启新路线，也不属于任何结局收集条件。

### `THE LOVERS`

D3 输入 `THE LOVERS` 后，以下六个好感度变量都会增加 5：
...
D1 开头，Roswell 会让 Dave 在饮料杯上输入一个名字。该输入不仅会改变紧接着的杯面回应，部分名字还会影响后续巴士司机的身份或角色好感度。

名字分支按照代码中的具体字符串匹配。为稳定触发对应内容，建议使用下文列出的**首字母大写拼写**。
...
这些分支只增加被输入角色本人的好感度，而不是同时增加六人的好感度。
...
输入 `Chase` 会触发与《Echo／回音镇》主角 Chase 有关的联动彩蛋。
...
`Rem` 同时出现在游戏鸣谢名单中，因此这一分支应属于面向支持者或相关人员的内部彩蛋。不过，目前没有进一步解释“几乎只有水”的具体梗来源。
...
Zylus 和 Wilson 所对应的具体社区成员或支持者身份，当前没有从明确的公开来源中得到完整确认。下文只记录游戏内可见内容，不将其现实身份作为确定结论。
:::

### 隐藏司机名字

输入以下三个名字，会影响稍后前来接送众人的巴士司机：
...
输入 `Wilson` 后，Dave 会发现杯子侧面写满了密集的小字和线条，看起来像一篇论文或某种思维导图。Roswell 猜测店员可能在制作饮料时突然产生了灵感。Dave 表示字太小，根本看不清究竟写了什么，最后喝一口确认里面至少还是咖啡。

稍后的默认司机会被替换为狐狸 **Wilson**，并显示其专属隐藏立绘。Roswell 把司机称为 Charon，Dave 也直接向“Charon”问好。司机试图纠正：“我的名字是 Wils——算了，这样也行。”
...
稍后的司机会被替换为体型非常高大的熊 **Timber**，并显示其专属隐藏立绘。他的身材甚至明显超过 Dean。Timber 下车确认目的地后，态度相当简短，直接让众人上车。Orlando 对他的巨大体型非常震惊，还拐弯抹角地猜测他是不是“哪里都很大”，Sal 则反复制止 Orlando。Dean 也承认 Timber 比自己家族中的大多数人都更高大，并略带竞争或嫉妒地暗示 Dave 有自己就已经“忙不过来”。
...
| `Rem` | 有 | 无 | 无 | 支持者彩蛋 |
| `Wilson` | 有 | 六人各 \(+1\) | Wilson | Charon 玩笑（可能为支持者彩蛋） |
...
- `CGdump`、`deanlove` 和不计入画廊的图片：见[CG 画廊查漏索引](../collectibles/gallery.md#不在画廊统计中的-cg)；
