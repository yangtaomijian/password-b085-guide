...
角色线一旦确认，在正常周目中便不会改变。之后通过 **Resonate?** 返回较早的密码检定或失败剧情时，也不会重新开放 D4 搭档选择，更不会自动更换角色线。
...
存档／读取（Save／Load）界面的整体主题不会随角色线改变，但每个存档槽会根据当前角色线显示不同的底色，并在右上区域显示对应角色的图标。
...
- 从 Path C 一侧开始的流程，也可能在 D14 的生死结果后转入 Path D。

因此，字母线描述的是当前时间线和后期主线走向，并不会在某次选择后永久固定为一条个人路线。

具体分流条件、Path E 的多入口结构，以及 Sal／Tyson 线的特殊豁免见[字母线系统](path-system.md)。

## 为什么存档显示的字母线可能滞后

存档槽不会提前显示 Dave 尚未得知的事件结果，因此字母线标签可能暂时落后于实际走向。

例如：

1. D8 选择 **Reveal Oz** 后，Oswin 早死的结果已经确定；
2. D10 正确完成密码检定时，当前字母线仍会先设为 A；
3. D11 中，在 Dave 发现 Oswin 的尸体之前保存，存档仍可能显示 `Path A`；
4. D11 晚上发现尸体后，游戏会立即把当前字母线改为 B；
5. 此后保存的存档已经会显示 `Path B`，D12 的日切画面只是下一次明确显示 B 的新日界面。

因此，字母线标签暂时和预期不一致，并不一定表示玩家走错了路线。之前的选择可能已经决定后续结果，但标签会等 Dave 得知发生了什么以后才更新。

::: {.callout-note}
## Sal 线的显示异常

Sal 线在 D10 放弃密码检定后，存档槽可能暂时显示 `Path C`；这是 b0.85 及更早版本的显示问题，剧情仍会进入共同的 Path A/B 流程。具体机制见[字母线系统](path-system.md)。
:::
...
| 字母线 | 主要时间线方向、角色生死结果、后期剧情结构和结局走向 |

:::

因此：

- 两名玩家可能处于同一条字母线，却因为选择了不同角色线而看到不同场景；
- 同一角色线的两个周目，也可能因为密码检定和关键选择不同而进入不同字母线。
