...
本站为非官方玩家整理，不是完整 Wiki，也不隶属于游戏作者或发行平台。内容以玩家实际可触发的游戏流程为主，并结合脚本核验区分正常节点、旧版残留与不可达代码。
...
强制密码通常按照一级、二级和三级提示逐步展开，不在普通页面直接展示完整答案。彩蛋密码、旧版本密码和不影响正常流程的代码残留会单独说明。
:::
...
本站在整理代码信息时，优先回答“玩家在正常游戏中实际能够触发什么”，而不是简单罗列脚本中出现过的变量和语句。

具体遵循以下原则：

- 区分正常流程、互斥选项、坏结局分支和不可达代码；
- 保留游戏中的英文选项、变量名和脚本标签，便于核对；
...
本站内容由原 B 站攻略专栏迁移、核对并重新组织而来。与早期专栏相比，当前网站更侧重可检索的路线结构、收集条件和机制说明，不再保留大篇幅剧情概览。

尽管内容经过实际游玩和脚本核验，仍可能存在遗漏、版本差异或对复杂控制流的误判。发现问题时，建议同时提供所在页面、游戏版本、触发路线和具体选项，便于复核。
