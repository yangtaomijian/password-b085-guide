...
《Password》b0.85 的六项好感度会改变部分对话、剧情和关系结果。

所有加点选项和完整清单见[好感度机制与加点](affection.md)。
...
- 差异只会在剧情到达对应检定时出现，而且通常还取决于角色线、日期、先前选择或其他剧情标记；多个门槛同时满足时，以较高门槛的结果为准。
:::
...
|---|---|---|---|
| `dragonlove >= 5` | D6 Orlando 晚餐后；D9 早晨 | D6 要求 Orlando 线和 D6 金库检定成功 | D6 中，Orlando 会以帮助 Dave 为 Dean 做准备为由提出接吻练习，并显示 `orlandokiss`。低于 5 时会跳过练习吻。D9 早晨，5—9 点会进入较犹豫的安慰剧情：Orlando 会询问自己能否帮忙，但仍显得犹豫。 |
...
| `dragonlove >= 10` | D9 早晨；D19 Orlando 厨房场景 | D9 达到 10 点时使用这一安慰剧情；D19 要求 Orlando 线，并进入 Dean 约会失败后通往厨房的分支 | D9 中，Orlando 会察觉 Dave 已接近崩溃，触碰他的脸，并追问是否有人伤害了他。D19 中，10—19 点会出现 `I love you too.`／`Stay quiet.`，让玩家接受或拒绝 Orlando。 |
...
| `dragonlove >= 20` | D18 Orlando 场景；两段 D19 Orlando 场景 | D18 变化还要求 D17 金库检定成功；D19 要求进入相应 Orlando 分支 | D18 中，Dave 会握住 Orlando 的手、把他拉近，并增强场景中的身体亲密。D19 的一段后续中会出现一个较小的相互触碰差异；在之后的厨房告白中，Orlando 会明确说爱 Dave，游戏自动进入 `OrlandoBoyfriend`。 |
...
| `bearlove >= 17` | D10 夜间 | Dean 线，且 D10 金库检定成功 | Dave 会意识到自己对 Dean 的感情可能已超过普通喜欢，并认为二人之后需要认真谈谈。 |
| `bearlove >= 20` | D18 会后 Dean 场景 | Dean 线，且 D17 金库检定成功 | Dave 会把 `I appreciate it` 改成更私人的 `I appreciate you`，明确期待次日约会，并主动开始更强烈的亲吻，之后才停下来休息。 |
...
| 检定 | 主要剧情位置 | 附加条件 | 对话或剧情效果 |
|---|---|---|---|
| `wolflove >= 10` | D9 早晨；D9 夜间 Tyson 事件；D19 关系检定 | D9 早晨中，达到 16 点时会使用另一段安慰剧情 | Dave 会直接请求拥抱，Tyson 会留下安慰他。D9 夜间，Tyson 会进一步解释自己的信任、被梳理毛发时的敏感，以及自己与狼族身份的关系。D19 中，10—19 点会出现 `I love you.`／`...`。 |
| `wolflove >= 15` | D9 夜间；D15 A/B；D16 A/B 早晨；D18 Tyson 场景 | 部分位置在达到 18 点时会使用另一段剧情 | D9 夜间，Dave 会说与 Tyson 一起待在家里可能比豪宅更重要。D15 梦中，他会说自己爱 Tyson，醒来后的对话中也几乎说出口。D16 早晨，Dave 开始把这种依恋理解为可能的恋爱，而不仅是兄弟情。D18 进一步增加二人关系究竟属于亲情还是恋爱的冲突。 |
| `wolflove >= 16` | D9 早晨 | 达到 16 点时替代 10 点剧情 | Dave 会直接说自己想要父亲。Tyson 会给出更完整的拥抱，并承认 Dave 的父亲对自己而言也几乎像父亲，使二人共同的失去被明确说出。 |
| `wolflove >= 17` | D9 Tyson 角色事件 | 到达关于 Dave 父亲的对话 | 增加一小段 Tyson 承认自己也希望获得父亲式建议的对白。 |
| `wolflove >= 18` | D9 夜间；D10 Tyson 调查；D16 A/B Tyson 事件 | D9 达到 18 点时替代 15 点剧情 | D9 夜间，Dave 会接近告白，并说即使这段关系让自己受伤，仍然想要 Tyson。D10 中，Tyson 会要求 Dave 答应无论发生什么都要保持幸福。D16 事件会加入更直接的身体吸引、关系焦虑和情感开放。 |
...
| `lionlove >= 16` | D10 Hoss 调查 | 只有此前已与 Hoss 接吻时才会使用 | 增加 Hoss 羡慕 Tyson 能公开关心 Dave 的对白。此前尚未接吻时，D10 是否出现亲吻由另一项剧情条件决定。 |
...
如果 D8 隐藏图书馆没有达到 12 点，`HossKiss` 会保持 `False`。之后若在 D9 夜晚前将 `lionlove` 提升到 12 或以上，Hoss 仍会在 D9 夜间场景中亲吻 Dave，并将 `HossKiss` 设为 `True`，但这不会补解锁已经错过的 D8 `hosskiss` CG。
...
| `croclove >= 15` | D9 早晨和 Sal 事件；D9 夜间；D15 A/B；D16 A/B；D18；D19 | 多数位置要求 Sal 线或相应 Sal 场景 | Sal 会更直接询问 Dave 为什么难过。D9 角色事件会扩展 Abi 相关对话，`Approach.` 也会成功变成拥抱，而不是被拒绝。D9 夜间 Sal 都会留宿；达到此门槛后，邀请和拥抱的对白会有所变化。D15—D18 会增加身体靠近、Sal 对 Dave 缺点的接受，以及爱 Dave 是否等于背叛 Dean 的冲突。D19 在未达到 20 时出现 `Romantic`／`Platonic`。 |
...
好感度没有统一的关系等级。同样的分数，在不同场景中会影响内心描写、亲密互动、可选对白或恋爱关系，不能简单对应成“朋友”“暗恋”“伴侣”。
:::
