...
- 本页只记录**好感度阈值造成的差异**。单纯由路线变量或其他剧情变量决定的内容，会在相关位置特别说明。
:::
...
Orlando 线的核心是从以 Dean 为借口的练习和撮合，逐渐转向 Dave 看穿 Orlando 内心的压抑与逃避。
...
| 检定条件 | 主要剧情节点 | 附加条件 | 实际文本或流程差异 |
|---|---|---|---|
| `bearlove >= 5` | D5 Dean 线 hot tub | `bearroute == True`，且电影选择 `Romance` | Dean 会把浪漫电影理解为 Dave 对自己的暗示，显示 `deankiss`，并出现是否亲吻 Dean 的菜单。低于 5 时仍有调情对白，但不会出现正式亲吻选择。需要注意，`deankiss` 在菜单出现前已经显示，因此即使之后选择 `No`，该 CG 也已经解锁。 |
| `bearlove >= 10` | D19 Dean 约会结束 | 进入 Dean date / post-date 流程 | Dave 会明确表示自己同样期待这次约会，并把对 Dean 的感觉描述为接近爱。脚本直接执行 `$ DaveBoyfriend = "Dean"`，随后进入正式恋爱及身体亲密场景。低于 10 时，Dave 会认为约会没有产生足够的火花，恋爱关系不会成立。 |
...
| `bearlove >= 17` | D10 夜间 | `bearroute == True` 且成功输入D10 密码 | Dave 会在内心承认自己对 Dean 的感情可能已经超过单纯的喜欢，并认为二人之后必须认真谈谈。 |
...
| `croclove >= 16` | D11 A／B 的 `Day11AB_SDO_Scenario` | 进入相应对话 | 被问是否有喜欢的人时，Sal 会承认确实有，而且是最近才意识到、仍在处理中的感情；他没有在这里明说对象是谁。低于 16 时，这一回答被省略。 |
...
因此，不应简单把 5、10、15、20 分别理解成统一的“朋友”“暧昧”“恋人”等阶段。
:::
