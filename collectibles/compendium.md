...
完成第一个结局后，后记按钮才会出现在**主菜单**。正常游戏流程中的导航菜单不会提供该入口。
...
b0.85 的角色档案列表没有 Oswin 条目。即使已经完成 Path A 后期的相关剧情，后记中也不会出现新的档案槽位，因此可见总数始终是 16。这不是漏收项目。
</details>
...
即使上一段已经完整播放，下一项也可能要到重启游戏后才显示。下一项仍是 `?????` 时，请先重启游戏；重启后仍未解锁，再重播上一段后日谈。
