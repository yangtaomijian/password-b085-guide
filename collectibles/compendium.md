...
D10 正确完成密码检定时，Sal 的角色档案会在相关场景中写入，**不要求当前为 Sal 线**。
...
b0.85 定义了 `persistent.oswin_lore`，并会在 Path A 后期把它设为 `True`，但 Oswin 唯一的角色档案条目已被禁用。
...
只有 Tyson 后日谈会直接检查 Path P 是否已经完成。后面的场景通过前一段后日谈的完成标记间接依赖 Path P。

### 每段后日谈都必须播放到完成节点

用于解锁下一段后日谈的完成标记，会在当前回放接近结尾时才写入。

如果在写入前使用 **End Replay（结束回放）** 退出，完成标记不会生效。仅仅打开回放，或只观看其中一部分，都不足以推进解锁链。
