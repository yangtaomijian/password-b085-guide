...
每个 Gallery 按钮对应一张登记图片。图片在剧情中实际出现后，Ren'Py 可以把它记录为 seen，并解锁对应按钮。

`Gallery Completion` 数字与按钮状态分别计算，并不是第二项解锁要求。

Gallery 进度独立于普通剧情存档；读取或删除普通存档，都不会清除已经记录为 seen 的图片。图片只有在实际显示后才会计入 seen；只存在于游戏文件、废弃分支或不可到达测试代码中的图片不会自动计入。
...
`Day10Morning2`、`Day11ABMeeting` 和 `FinalPassword` 等名称是标出 CG 出现位置的脚本 label，游戏界面中不会显示。

- 表格中的日期和位置用于找到对应场景，不代表进入该分支所需的全部前置条件；
- “上下文”摘录 CG 出现前后的剧情文本，方便确认场景；
...
下方索引按角色列出每张登记图片的 CG ID、最早正常出现位置和附近剧情，方便从 Gallery 格子反查对应场景。
