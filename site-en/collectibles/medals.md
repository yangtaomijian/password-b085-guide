---
title: "Twelve-Medal Collection Guide"
description: "How to collect all twelve zodiac medals and unlock the Path P sequence in Password b0.85"
toc: true
---
...
- All twelve zodiac medals are required for the Path P sequence.
- Medal progress is stored in persistent data and is not rolled back when you load an older save.
- Four character routes have exclusive medals:
  - Dean Route: Pisces
  - Roswell Route: Cancer
  - Sal Route: Capricorn
  - Hoss Route: Libra
...
A medal can appear in the story before the game records it as a persistent collectible.
...
| Category | Medals |
|---|---|
| A/B-side unlocks | Aquarius, Taurus, Leo |
| Character-route medals | Pisces, Cancer, Capricorn, Libra |
| Path A medals | Aries, Scorpio, Virgo |
| Path B medals | Gemini, Sagittarius |
...
Choose Dean, Roswell, Sal, or Hoss and complete Path B.

That run provides six medals:
...
- Gemini
- Sagittarius
- the exclusive medal for the selected character route

Because these are non-Tyson routes, the D17 password must be handled correctly to avoid being diverted into Path F/G.

### Step 3: Collect two more route medals
...
Use the one collectible route not yet completed and finish Path A.

This final run provides:

- the remaining character-route medal;
- Aries;
- Scorpio;
- Virgo.

The successful twelve-medal check occurs only after the Path A ending, so placing Path A last avoids having to complete it a second time.

### Two efficient patterns

::: {.medal-route-plan-table .table-responsive .table-scroll-compact}

| Pattern | Path B run | Short collection runs | Final Path A run |
|---|---|---|---|
| A | Hoss Route | Two of Dean, Roswell, and Sal to D9 | The remaining D9 route |
| B | Dean, Roswell, or Sal Route | The other two D9 routes | Hoss Route to D16 and then Path A |
...
| Medal | Requirement | Story location | Persistent unlock |
|---|---|---|---|
| **Aquarius** | Path A or B side | Orlando finds it in the D4 hedge maze | D16 A/B inventory scene |
| **Taurus** | Path A or B side | Found under the giant pumpkin on D7 | D16 A/B inventory scene |
| **Gemini** | Path B | The two halves are reunited on D25 B | D25 B |
| **Cancer** | Roswell Route | Museum book scene on D9 | D9 |
| **Leo** | Path A or B | Hidden inside a hollowed-out children's book on D22 | D22 |
| **Virgo** | Complete Path A | Florencia gives it to Dave during the Path A ending | Path A ending |
| **Libra** | Hoss Route + Path A or B | Hidden library on D16 | D16 |
| **Scorpio** | Path A | Found inside Roswell's letter on D24 A | D24 A |
| **Sagittarius** | Complete Path B | Florencia gives it to Dave during the Path B ending | Path B ending |
| **Capricorn** | Sal Route | Pool locker scene on D9 | D9 |
| **Aries** | Path A | Roswell's bedside table on D23 | D23 |
| **Pisces** | Dean Route | River fishing scene on D9 | D9 |

:::

## Detailed medal notes
...
## Why twelve medals may not trigger Path P
...
5. Did that run continue through the Path B ending, where Sagittarius is awarded?
6. Does the Compendium still show an unidentified medal entry after restarting the game?
7. Was persistent data cleared, replaced, or incompletely transferred between devices?
