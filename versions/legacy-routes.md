...
这是一个终止周目的坏结局。它不会开启新的角色线或字母线，也不会再回到正常主线。

b0.85 已删除正常可达的 D6 离开宅邸菜单，以及对应的坏结局剧本。
...
坏结局中的版本不会写入正常流程使用的 `OzKnown` 状态，也不会决定 Path A 或 Path B。
:::
...
b0.85 在后续 Path P 中新增了独立输入，并加入明确的真结局跨存档标记。相关变化见[b0.85 版本主要变化](b085-changes.md)。
...
| Path A 首次流程 | 由跨存档共享的变量控制；b0.85 后来加入追加场景回放 | [旧版本机制档案](legacy-mechanics.md) |
