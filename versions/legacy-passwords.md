...
::: {.legacy-password-overview-table}
...
旧版 Path A 的 D11 曾存在一个非强制密码 `METEMPSYCHOSIS` ，它与四个强制关键密码不同，类似彩蛋密码性质：

- 不输入也可以继续主线；
- 输入与否不会改变最终字母/角色线；
- 仅改变当天晚上 Dave 与 Oswin 的部分对话和实验室信息；
- 更接近隐藏文本入口，而不是通关检定。

### 旧版输入时间

只有以下全部条件全部满足时才可输入该密码：

- 任意角色线；
- Path A；
- 非首次经历该段路线（ `persistent` 变量未记录过）；
- D11 的“前往金库”选项中。
