...
b0.85 将它改为固定流程：Oswin 从背后给 Dave 注射，之后再说明注射物其实是生理盐水。这项重写删除了玩家选择的机会，也删除了原本可能取得的 `DavePride` 增量。
...
两张 Tyson 相关 CG（`tysondrive` 和 `tysonmovie`）在 b0.7 与 b0.85 之间被完整重绘。
