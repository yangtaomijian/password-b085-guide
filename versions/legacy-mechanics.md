...
旧变量、脚本标签、密码字符串或条件判断仍留在源码中，不代表相应机制在 b0.85 还能使用。本页涉及的部分旧机制只剩零散代码，当前剧情或界面已经没有完整入口。
:::
...
这是跨存档共享的变量，而不是普通存档变量。它会在不同角色线和存档槽之间共享。
...
因此，第一次流程并不会在写入跨存档状态后结束。后面的 `Day23A_Prime` 内容会直接接在同一次长流程后继续播放。
...
单纯开始 **New Game** 不会重置这个跨存档状态。只有重置或删除游戏的持久数据，才会恢复默认的首次流程状态。

### 读取旧存档

结果取决于存档位置：

- 读取在跨存档状态分发逻辑**之前**保存的存档时，游戏会重新读取当前跨存档共享的状态，因此可能直接跳过首次段落；
- 读取已经位于首次段落**内部**的存档时，该次流程早已通过分发逻辑，可以从保存位置继续。
...
`persistent.Day23APrime` 作为解锁条件，因此通常会一起解锁。当前列表位置和解锁排查见[后记解锁索引](../collectibles/compendium.md)。

### 回放只覆盖原剧情的一部分

两个条目复用了原有剧情脚本标签，但并不等同于重新播放完整的正常 Path A 流程。
...
- 直接从 Roswell 段落开始；
- 在正常流程写入跨存档状态以及继续进入 `Day23A_Prime` 之前结束。

这些回放仍可能显示已注册的画廊图片，也会执行各自范围内的变量赋值。不过两段回放都有明确的起止点：它们不是独立视频，也不会重现完整的正常流程及周边所有剧情效果。
...
|---|---|
| 大小写 | 会统一转为小写，因此不敏感 |
| 开头或结尾空格 | 源码不会主动去除，但许多输入仍可匹配 |
| 常见 ASCII 标点 | 固定列表内的标点会被移除 |
| 撇号和连字符 | 不在该删除列表中 |
| Unicode 标点 | 不会被统一处理 |
| 多个连续空格 | 可能破坏多词片段匹配 |
| 关键词组顺序 | 通常不作强制要求 |
| 旧清单中的 `/` | 只是编辑分隔符，没有解析含义 |
:::
...
这张表不是完整输入总表，但列出的句子都可以直接尝试。部分话题还有其他可用写法。
...
- 固定对白复用了部分旧话题；
- 某些旧普通变量和后续条件判断仍留在源码；
- 正常输入菜单、`renpy.input` 循环、关键词引擎和旧回应脚本标签已不再向玩家开放。
