...
旧变量、label、密码字符串或条件判断仍留在源码中，不代表相应机制在 b0.85 还能使用。本页涉及的部分旧机制只剩零散代码，当前剧情或界面已经没有完整入口。
:::
...
这些 replay 仍可能显示已注册的 Gallery 图片，也会执行各自范围内的变量赋值。不过两段回放都有明确的起止点：它们不是独立视频，也不会重现完整的正常流程及周边所有剧情效果。
...
- 固定对白复用了部分旧话题；
- 某些旧普通变量和后续条件判断仍留在源码；
- 正常输入菜单、`renpy.input` 循环、关键词引擎和旧回应 labels 已不再向玩家开放。

Cast Files、Lore 和 Additional Scenes 中也没有提供这套完整问答库。
