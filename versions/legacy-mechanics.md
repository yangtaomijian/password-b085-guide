...
旧密码字符串或状态仍有残留，不代表相应机制在 b0.85 还能使用。本页涉及的部分旧机制只剩零散痕迹，当前剧情或界面已经没有完整入口。
:::
...
因此，第一次流程不会在两段特殊剧情结束后中断，而是会直接接着播放 Path A 的后续内容。游戏会用 `persistent.Day23APrime` 记录这次完成状态。
...
两个条目只回放原剧情中的限定片段，并不等同于重新播放完整的 Path A 流程。

`Dave's Demise` 从 Dave 的对应剧情开始，并在进入 Roswell 段落前结束。

`Roswell's Attempt` 从 Roswell 段落开始，并在 Path A 后续剧情继续前结束。

回放仍可解锁其中出现的画廊图片，也会保留片段本身的剧情效果。不过两段回放都有明确的起止点：它们不是独立视频，也不会重现完整的 Path A 流程及周边剧情。
