...
两项都以 `persistent.Day23APrime` 为解锁条件，会一起解锁。当前列表位置和解锁排查见[后记解锁索引](../collectibles/compendium.md)。
...
这段问答会通过 `OzPast1` 和 `OzPast2` 改变后续部分对白，不影响角色线、字母线、奖牌、主要结局或主金库密码的成功状态。
...
原先两种 `DavePride` 加点随之取消，角色线、字母线和结局不受影响。
